 ...
 *********************************************************************** */
{
  ...
  g_smallPressure = v[RHO]*star.temperature/(KELVIN*star.mean_mol);
...
# if COOLING !=NO
  ...
  g_minCoolingTemp = star.temperature;
...
# endif 
...
#if EOS == ISOTHERMAL                                                  
  g_isoSoundSpeed = star.sound_speed;
#endif

#if ROTATING_FRAME == YES                                                          
  g_OmegaZ = star.rotational_velocity;
#endif
  ...
  if (r > star.radius) {
    ...
    velocity = star.terminal_velocity
               *pow(1.0 - star.radius/r, star.vel_law_exponent);

    v[RHO] = (star.mass_loss/(4.0*CONST_PI*velocity*r2));
    ...
    v[RHO] = (star.mass_loss
               /(4.0*CONST_PI*(star.sound_speed
               /star.surface_rho_param)));

    velocity = star.terminal_velocity
                *pow(1.0 - (1.0/1.001), star.vel_law_exponent);
...
#if EOS == IDEAL
  v[PRS] = (v[RHO]*star.temperature/(KELVIN*star.mean_mol));
#endif                                                               

#if PHYSICS == MHD                                   
#if BACKGROUND_FIELD == NO
  InitMagneticField(magnetic_field, x1, x2, x3, star);
