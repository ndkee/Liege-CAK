...
void UserDefBoundary (const Data *d, RBox *box, int side, Grid *grid) {        
  ...
  double Edd, L, c, M_dot, M_dot_cgs, ke, Omega2, A, Bcgs, cs, Bq, v_esc, v_inf;
...
#endif
{
  ...
  if (r >= 1.0){ // - External gravity + centrifugal + coriolis 
    ...
    g[JDIR] = gla[1] + gg*x2/r + Fin_x2;
    g[KDIR] = gla[2] + gg*x3/r;
  } else if (r < 1.0) { // - Star interal gravity + rotation 
