...
/* -- initialize members of Time_Step structure -- */
  ...
  g_stepNumber = 0;
    ...
    if(g_inputParam[CAK_ifrc] < 0.0){
        // winflo sets the rotation of the stellar photosphere ... to be implemented
        //wo    = 0.5 * (pow(g_inputParam[winflo], 2) * grd[IDIR].x[IBEG]/(G_GravityConstant*M_X1_BEG));
        double wo    = 0.0;
        double arot  = wo * pow(1.0 - wo, 2);
        double Omega = sqrt(27.0 * arot/4.0);
        StartUpGCAK(&data, grd, Omega);
    }
